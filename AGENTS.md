...
- Softfolk is a public-domain collection of 16 identities with five costumes each: `signature`, `everyday`, `work`, `formal`, and `yukata`.
...
- Keep exactly 80 nested portrait files and no root-level aliases.
...
- `data/avatars.json` owns the 16×5 identity, tag, costume, and path inventory. Keep its paths aligned with `src/collection.ts` and the files on disk.
